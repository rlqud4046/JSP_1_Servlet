...
public class AdderServlet extends HttpServlet {
    ...
    /*
     * 요청과 관련된 API : javaw.servlet.http.HttpServletRequest 클래스
     * 응답과 관련된 API : javaw.servlet.http.HttpServletResponse 클래스
     * 1. 클라이언트가 서블릿에 요청을 하면 먼저 톰캣 서버가 해당 요청을 받음
     * 2. 그런 다음 사용자의 요청이나 응답에 대한 HttpServletRequest 객체와
     * 		HttpServletResponse 객체를 만들게 함
     * 3. 그리고 난 후 Servlet의 doGet() 메서드나 doPost()메서드를 호출하면서 이 객체들을 전달
     * 4. 톰캣이 사용자의 요청에 대한 정보를 모든 HttpServletRequest 객체의 속성으로 담아 메서드로 전달함.
     * 		따라서 각 HttpServletRequest에서 제공하는 메서드들은 매개변수로 넘어온 객체들을 이용하여
     * 		사용자가 전송한 데이터를 받아오거나 응답할 수 있는 것임
     * 서블릿에서 클라이언트의 요청을 얻는 방법
     * - HttpServletRequest 클래스에서<form> 태그로 전송된 데이터를 받아오는데 사용하는 메서드
     * 		*getParameter(String name) => <form>태그의 name 속성에 들어간 변수명을  받아서 사용함 반환형은 String
     * 		*getParameterValues(String name) => 같은 name에 대하여 여러 개의 값을 얻을 때 사용함. 반환형은 String[]
     * 
     ...
     * 4. setContentType() 메서드를 이용하여 클라이언트에게 전송함
     ...
     * 1. GET 방식
     * 		- 서블릿에서 데이터를 전송할 때는 데이터가 url 뒤에 name=value 형태로 전송이 됨
     ...
     * 		
     * 2. POST 방식
     * 		- 서블릿에 데이터를 전송할 때는 TCP/IP 프로토콜 데이터의 head 여역에 숨겨진 채 전송이 됨
     * 		- 보안에 유리함
     * 		- 전송 데이터의 용량이 무제한임
     * 		- 처리 속도가 get 방식보다 아주조금 느림
